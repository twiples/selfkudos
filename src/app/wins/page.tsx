      ...
      {sortedGroups.length > 0 ? (
        <div className="space-y-8">
          {sortedGroups.map(([key, group]) => (
            <div key={key}>
              <h2 className="text-sm font-medium text-calm-500 mb-3">{group.label}</h2>
        ...
        <div className="text-center py-12 bg-calm-50 rounded-2xl">
          <h2 className="text-lg font-semibold text-calm-800 mb-2">No wins recorded yet</h2>
...
function WinCard({ win, onDelete }: { win: PrivateWin; onDelete: () => void }) {
  ...
  return (
    <div className="bg-white rounded-xl p-5 border border-calm-200">
      <p className="text-calm-800">{win.content}</p>

      {win.significance && (
        <p className="text-calm-600 text-sm mt-3 italic">
